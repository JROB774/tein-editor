...
GonObject LoadFromTokens(GonTokenStream& Tokens){
    GonObject ret;
    ...
    if(Tokens.Peek() == "{"){         //read object
        ...
        while(Tokens.Peek() != "}"){
            //while(Tokens.Peek() == ";") Tokens.Read(); //ignore out of place ;
            ...
            /*if(Tokens.Peek() == ";"){
                Tokens.Read(); //consume ;
                GonObject empty;
                empty.type = GonObject::g_null;
                ret.children_array.push_back(empty);
            } else {*/

...
FILDEF void create_new_map_tab_and_focus ()
{
    ...
    tab.map_node_info.active_pos = { 0,0 };
...
FILDEF void close_tab (size_t _index)
{
    ...
    if (editor.closed_tabs.empty() || editor.closed_tabs.back() != editor.tabs.at(_index).name) {
        editor.closed_tabs.push_back(editor.tabs.at(_index).name);
    }
...
FILDEF void backup_tab (Tab& _tab)
{
    ...
    case (TAB_TYPE_MAP  ): { backup_map_tab  (_tab,       _tab.name); } break;
    }
}

...
FILDEF void do_hotbar ()
{
    ...
    if (are_there_any_tabs()) {
        ...
        if (tab.type == TAB_TYPE_LEVEL) {
            if (tab.level_history.state.empty()) { undo_flags = UI_LOCKED; redo_flags = UI_LOCKED; }
            if (tab.level_history.current_position <= -1)                                          { undo_flags = UI_LOCKED; }
            if (tab.level_history.current_position == CAST(int, tab.level_history.state.size())-1) { redo_flags = UI_LOCKED; }
        } else {
            if (tab.map_history.state.size()==1) { undo_flags = UI_LOCKED; redo_flags = UI_LOCKED; }
            if (tab.map_history.current_position <= 0)                                         { undo_flags = UI_LOCKED; }
            if (tab.map_history.current_position == CAST(int, tab.map_history.state.size())-1) { redo_flags = UI_LOCKED; }
        }
...
FILDEF void hb_load ()
{
    std::vector<std::string> file_names = open_dialog(DIALOG_TYPE_LVL_AND_CSV);
...
FILDEF void hb_save ()
{
    ...
    case (TAB_TYPE_LEVEL): { le_save     (get_current_tab()); } break;
    case (TAB_TYPE_MAP  ): { save_map_tab(get_current_tab()); } break;
    }
}
...
FILDEF void hb_save_as ()
{
    ...
    switch (get_current_tab().type) {
    case (TAB_TYPE_LEVEL): { le_save_as     (); } break;
    case (TAB_TYPE_MAP  ): { save_map_tab_as(); } break;
    }
}
...
FILDEF void hb_undo_action ()
{
    ...
    switch (get_current_tab().type) {
    case (TAB_TYPE_LEVEL): { le_undo(); } break;
    case (TAB_TYPE_MAP  ): { me_undo(); } break;
    }
}
...
FILDEF void hb_redo_action ()
{
    ...
    switch (get_current_tab().type) {
    case (TAB_TYPE_LEVEL): { le_redo(); } break;
    case (TAB_TYPE_MAP  ): { me_redo(); } break;
    }
}
...
FILDEF void hb_history_begin ()
{
    ...
    switch (get_current_tab().type) {
    case (TAB_TYPE_LEVEL): { le_history_begin(); } break;
    case (TAB_TYPE_MAP  ): { me_history_begin(); } break;
    }
}
...
FILDEF void hb_history_end ()
{
    ...
    switch (get_current_tab().type) {
    case (TAB_TYPE_LEVEL): { le_history_end(); } break;
    case (TAB_TYPE_MAP  ): { me_history_end(); } break;
    }
}
...
FILDEF void hb_zoom_out ()
{
    ...
    if (tab.type == TAB_TYPE_LEVEL) {
        if ((tab.camera.zoom /= 2.0f) < MIN_LVL_EDITOR_ZOOM) {
            tab.camera.zoom = MIN_LVL_EDITOR_ZOOM;
        }
    } else {
        if ((tab.camera.zoom /= 2.0f) < MIN_MAP_EDITOR_ZOOM) {
...
FILDEF void hb_zoom_in ()
{
    ...
    Tab& tab = get_current_tab();
    if (tab.type == TAB_TYPE_LEVEL) {
        if ((tab.camera.zoom *= 2.0f) > MAX_LVL_EDITOR_ZOOM) {
            tab.camera.zoom = MAX_LVL_EDITOR_ZOOM;
        }
    } else {
        if ((tab.camera.zoom *= 2.0f) > MAX_MAP_EDITOR_ZOOM) {
...
FILDEF void hb_run_game ()
{
    ...
    {
    // Steam
    "C:/Program Files (x86)/Steam/steamapps/common/theendisnigh/TheEndIsNigh.exe",
    "C:/Program Files/Steam/steamapps/common/theendisnigh/TheEndIsNigh.exe",
    "TheEndIsNigh.exe",
    // Epic
    "C:/Program Files (x86)/Epic Games/theendisnigh/TheEnd.exe",
    "C:/Program Files)/Epic Games/theendisnigh/TheEnd.exe",
    "TheEnd.exe"
    };

...
FILDEF bool internal__load_map (Map& _map, std::istream&& _stream)
{
...
FILDEF bool internal__save_map (const Map& _map, FILE* _file)
{
    // If the map is empty just save an empty file.
    if (_map.empty()) { return true; }

    int x = get_map_x_pos (_map);
    int y = get_map_y_pos (_map);
    int w = get_map_width (_map);
    int h = get_map_height(_map);
        ...
        for (int ix=x; ix<w; ++ix) {
            for (auto& node: _map) {
                if (node.x == ix && node.y == iy) {
                    ...
                    if (node.lvl.find(',') == std::string::npos) {
                        fprintf(_file, "%s", node.lvl.c_str());
                    } else {
                        fprintf(_file, "\"%s\"", node.lvl.c_str());
                    }
                    break;
...
STDDEF bool load_map (Map& _map, const char* _file_name)
{
    ...
    return internal__load_map(_map, std::ifstream(_file_name));
}

STDDEF bool save_map (const Map& _map, const char* _file_name)
{
    // We don't make the path absolute or anything becuase if that is needed
    // then it should be handled by a higher-level than this internal system.
    ...
    return internal__save_map(_map, file);
}
...
STDDEF bool load_restore_map (Tab& _tab, const char* _file_name)
{
    ...
    return internal__load_map(_tab.map, std::istringstream(data.substr(map_name.length()+1)));
}
...
STDDEF bool save_restore_map (const Tab& _tab, const char* _file_name)
{
    ...
    return internal__save_map(_tab.map, file);
}

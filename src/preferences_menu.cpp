...
GLOBAL constexpr float PREFERENCES_SECTION_H       = 24.0f;
GLOBAL constexpr float PREFERENCES_INNER_XPAD      = 10.0f;
GLOBAL constexpr float PREFERENCES_INNER_YPAD      =  5.0f;
GLOBAL constexpr float PREFERENCES_TEXT_BOX_INSET  =  2.0f;
GLOBAL constexpr float PREFERENCES_SCROLLBAR_WIDTH = 12.0f;
GLOBAL constexpr float PREFERENCES_COLOR_HEIGHT    = 18.0f;

GLOBAL constexpr u32 PREFERENCES_TAB_SETTINGS = 1;
GLOBAL constexpr u32 PREFERENCES_TAB_HOTKEYS  = 2;

GLOBAL u32 preferences_tab = PREFERENCES_TAB_SETTINGS;

GLOBAL float preferences_scroll_offset = 0.0f;
...
FILDEF void internal__next_section (Vec2& _cursor)
{
    _cursor.x  = 0.0f;
    _cursor.y += PREFERENCES_SECTION_H;
}

FILDEF void internal__do_settings_label (float _w, const char* _key)
{
    do_label(UI_ALIGN_LEFT,UI_ALIGN_CENTER, _w,PREFERENCES_SECTION_H, PREFERENCES_SETTINGS_NAMES.at(_key));

    advance_panel_cursor(PREFERENCES_INNER_XPAD/2.0f);
    do_separator(PREFERENCES_SECTION_H);
    advance_panel_cursor(PREFERENCES_INNER_XPAD/2.0f);
}

FILDEF void internal__do_half_settings_label (float _w, const char* _key)
{
    do_label(UI_ALIGN_LEFT,UI_ALIGN_CENTER, _w,PREFERENCES_SECTION_H, PREFERENCES_SETTINGS_NAMES.at(_key));
    advance_panel_cursor(PREFERENCES_INNER_XPAD/2.0f);
}

FILDEF void internal__do_half_separator (Vec2& _cursor)
{
    advance_panel_cursor((PREFERENCES_INNER_XPAD/2.0f)+1.0f);
    do_separator(PREFERENCES_SECTION_H);
    _cursor.x -= 1.0f;
    advance_panel_cursor(PREFERENCES_INNER_XPAD/2.0f);
}

FILDEF void internal__do_hotkeys_label (float _w, const char* _key)
{
    do_label(UI_ALIGN_LEFT,UI_ALIGN_CENTER, _w,PREFERENCES_SECTION_H, PREFERENCES_HOTKEYS_NAMES.at(_key));
...
FILDEF void internal__begin_settings_area (const char* _title, Vec2& _cursor)
{
    float w = get_viewport().w;
    float h = PREFERENCES_SECTION_H;

    do_label(UI_ALIGN_CENTER,UI_ALIGN_CENTER, w,h, _title);
    internal__next_section(_cursor);
...
FILDEF void internal__do_settings_color_swatch (Vec2& _cursor, float _sw, float _sh, Vec4& _color)
{
    // If we were presses we want to open the color picker with our color.
    if (preferences_mouse_pressed && mouse_in_ui_bounds_xywh(_cursor.x, _cursor.y, _sw, _sh)) {
        open_color_picker(&_color);
    }

    _cursor.y += 3.0f;
    set_draw_color(ui_color_light);
    fill_quad(_cursor.x+0.0f, _cursor.y+0.0f, _cursor.x+_sw-0.0f, _cursor.y+_sh-0.0f);
    set_draw_color(ui_color_ex_dark);
    fill_quad(_cursor.x+1.0f, _cursor.y+1.0f, _cursor.x+_sw-1.0f, _cursor.y+_sh-1.0f);

    float x1 = _cursor.x    +2.0f;
    float y1 = _cursor.y    +2.0f;
    float x2 = _cursor.x+_sw-2.0f;
    float y2 = _cursor.y+_sh-2.0f;
    ...
    Quad clip = { 0.0f, 0.0f, tw, th };
    draw_texture(tex, tx, ty, &clip);

    Vec4 max = { _color.r, _color.g, _color.b,     1.0f };
    Vec4 min = { _color.r, _color.g, _color.b, _color.a };
    ...
    _cursor.y -= 3.0f;
    advance_panel_cursor(_sw);
}

FILDEF void internal__do_color_setting_row (Vec2& _cursor, float _w, const char* _key1, const char* _key2, Vec4& _c1, Vec4& _c2)
{
    float lw1 = _w-1;
    float lw2 = _w;
    ...
    internal__do_half_settings_label(lw1, _key1);
    internal__do_settings_color_swatch(_cursor, sw, sh, _c1);
    internal__do_half_separator(_cursor);
    internal__do_half_settings_label(lw2, _key2);
    internal__do_settings_color_swatch(_cursor, sw, sh, _c2);
    internal__next_section(_cursor);
}

FILDEF void internal__do_hotkey_label_titles (Vec2& _cursor)
{
    float vw = get_viewport().w-(PREFERENCES_SCROLLBAR_WIDTH-1.0f);
    float sh = PREFERENCES_SECTION_H+(PREFERENCES_INNER_YPAD*2.0F);

    // We use get_viewport().w so the labels are slightly larger and align.
    float lw1 = (roundf((get_viewport().w-(PREFERENCES_INNER_XPAD*2)) / 3.0f) - (PREFERENCES_INNER_XPAD/2));
    float lw2 = roundf((vw-(lw1*2.0f))-roundf((PREFERENCES_INNER_XPAD*2.5f)+2.0f));

    do_quad(vw, sh, ui_color_medium);

    _cursor.x  = PREFERENCES_INNER_XPAD;
    _cursor.y += PREFERENCES_INNER_YPAD;

    do_label(UI_ALIGN_LEFT,UI_ALIGN_CENTER, lw1,PREFERENCES_SECTION_H, "Hotkey Action");

    advance_panel_cursor(PREFERENCES_INNER_XPAD/2.0f);
    advance_panel_cursor(1.0f);
    advance_panel_cursor(PREFERENCES_INNER_XPAD/2.0f);

    do_label(UI_ALIGN_RIGHT,UI_ALIGN_CENTER, lw2,PREFERENCES_SECTION_H, "Main Binding");
    _cursor.x += roundf(PREFERENCES_INNER_XPAD/3.0f);
...
FILDEF void internal__do_hotkey_rebind (Vec2& _cursor, const char* _key)
{
    float vw = get_viewport().w-(PREFERENCES_SCROLLBAR_WIDTH-1.0f);
    float sh = PREFERENCES_SECTION_H;

    // We use get_viewport().w so the labels are slightly larger and align.
    float lw = (roundf((get_viewport().w-(PREFERENCES_INNER_XPAD*2)) / 3.0f) - (PREFERENCES_INNER_XPAD/2));
    float tw = roundf((vw-(lw*2.0f))-roundf((PREFERENCES_INNER_XPAD*2.5f)+2.0f));

    do_quad(vw, sh, ui_color_medium);
    _cursor.x = PREFERENCES_INNER_XPAD;

    internal__do_hotkeys_label(lw, _key);

    // It says text box inset but we're using for the hotkey rebind as well...
    float th = sh-(PREFERENCES_TEXT_BOX_INSET*2.0f);
    _cursor.y += PREFERENCES_TEXT_BOX_INSET;
    do_hotkey_rebind_main(tw,th, UI_NONE, key_bindings.at(_key));
    _cursor.x += roundf(PREFERENCES_INNER_XPAD/3.0f);
    do_hotkey_rebind_alt(tw,th, UI_NONE, key_bindings.at(_key));
    _cursor.y -= PREFERENCES_TEXT_BOX_INSET;
...
FILDEF void internal__do_preferences_settings ()
{
    ...
    begin_panel(0.0f, 0.0f, vw, vh, UI_NONE, ui_color_medium);
    ...
    float sw = roundf(vw / 2.0f) - (xpad/2.0f);
    float sh = PREFERENCES_SECTION_H;

    float bw = roundf(sw / 2.0f);
    float th = sh-(PREFERENCES_TEXT_BOX_INSET*2.0f);
    ...
    float hw = roundf(vw / 4.0f) - (xpad/2.0f);
...
FILDEF void internal__do_preferences_hotkeys ()
{
    ...
    begin_panel(0.0f, 0.0f, vw, vh, UI_NONE);
    ...
    float content_height = (PREFERENCES_HOTKEYS_NAMES.size() * (PREFERENCES_SECTION_H+1.0f)) + (PREFERENCES_SECTION_H+(PREFERENCES_INNER_YPAD*2.0f));
    do_scrollbar(vw-PREFERENCES_SCROLLBAR_WIDTH+1.0f, -1.0f, PREFERENCES_SCROLLBAR_WIDTH, vh+2.0f, content_height, preferences_scroll_offset);
...
FILDEF void do_preferences_menu ()
{
    ...
    cursor = { 0.0f, 0.0f };
    begin_panel(0.0f, 0.0f, vw, pvfh, UI_NONE, ui_color_medium);

    set_panel_cursor_dir(UI_DIR_RIGHT);
    set_panel_cursor(&cursor);

    UI_Flag settings_flags = (preferences_tab == PREFERENCES_TAB_SETTINGS) ? UI_HIGHLIGHT : UI_INACTIVE;
    UI_Flag hotkeys_flags  = (preferences_tab == PREFERENCES_TAB_HOTKEYS)  ? UI_HIGHLIGHT : UI_INACTIVE;

    if (do_button_txt(NULL, tw,th, settings_flags, "Settings")) { preferences_tab = PREFERENCES_TAB_SETTINGS; }
    if (do_button_txt(NULL, tw,th, hotkeys_flags,  "Hotkeys"))  { preferences_tab = PREFERENCES_TAB_HOTKEYS;  }
    ...
    begin_panel(0.0f, vh-pvfh, vw, pvfh, UI_NONE, ui_color_medium);
    ...
    case (PREFERENCES_TAB_SETTINGS): { restore_message = "Restore Settings"; } break;
    case (PREFERENCES_TAB_HOTKEYS):  { restore_message = "Restore Hotkeys";  } break;
    }
    ...
    if (do_button_txt(NULL, bw ,bh, UI_NONE, restore_message.c_str())) { restore_preferences(); }
    ...
    p2.h = get_viewport().h - p2.y - pvfh - 1.0f;

    begin_panel(p2, UI_NONE);
    switch (preferences_tab) {
    case (PREFERENCES_TAB_SETTINGS): { internal__do_preferences_settings(); } break;
    case (PREFERENCES_TAB_HOTKEYS):  { internal__do_preferences_hotkeys();  } break;
    }
    end_panel();
...
{
    switch (preferences_tab) {
    case (PREFERENCES_TAB_SETTINGS): { restore_editor_settings();     } break;
    case (PREFERENCES_TAB_HOTKEYS):  { restore_editor_key_bindings(); } break;
    }
}
...
FILDEF void cancel_preferences ()
{
    ...
    if (editor_settings != cached_editor_settings || key_bindings != cached_editor_hotkeys) {
        ...
        if (show_alert("Discard Changes", "Are you sure you want do discard changes?",
          ALERT_TYPE_WARNING, ALERT_BUTTON_YES_NO, "WINPREFERENCES") == ALERT_RESULT_NO) {
            return;
        }
    }
...
FILDEF void save_preferences ()
{
    ...
    if (editor_settings != cached_editor_settings || key_bindings != cached_editor_hotkeys) { internal__save_settings(); }

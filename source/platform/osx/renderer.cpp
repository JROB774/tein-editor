...
GLOBAL std::stack<mat4> projection_stack;
GLOBAL std::stack<mat4> modelview_stack;
...
FILDEF void internal__set_projection_uniform (Shader shader)
{
    ...
    glUniformMatrix4fv(location, 1, GL_FALSE, &projection_stack.top()[0][0]);
}
FILDEF void internal__set_modelview_uniform (Shader shader)
{
    GLint location = glGetUniformLocation(shader, "modelview");
    glUniformMatrix4fv(location, 1, GL_FALSE, &modelview_stack.top()[0][0]);
}
...
FILDEF bool init_renderer ()
{
    ...
    untextured_shader = load_shader_resource("shaders/330/untextured.shader");
    ...
    textured_shader = load_shader_resource("shaders/330/textured.shader");
    if (!textured_shader)
    {
    ...
    text_shader = load_shader_resource("shaders/330/text.shader");
    if (!text_shader)
    {
        LOG_ERROR(ERR_MED, "Failed to load the text shader!");
    }

    // We always have one matrix in each stack!
    projection_stack.push(mat4(1));
...
STDDEF vec2 screen_to_world (vec2 screen)
{
    ...
    mat4 projection = projection_stack.top();
    mat4 modelview = modelview_stack.top();
...
{
    projection_stack.top() = glm::ortho(l,r,b,t);
    modelview_stack.top() = mat4(1);
}
...
FILDEF void draw_line (float x1, float y1, float x2, float y2)
{
    ...
    internal__set_projection_uniform(untextured_shader);
    internal__set_modelview_uniform(untextured_shader);
...
FILDEF void push_matrix (Matrix_Mode mode)
{
    switch (mode)
    {
        case (Matrix_Mode::PROJECTION): projection_stack.push(mat4(1)); break;
        case (Matrix_Mode::MODELVIEW): modelview_stack.push(mat4(1)); break;
    }
}
FILDEF void pop_matrix (Matrix_Mode mode)
{
    switch (mode)
    {
        case (Matrix_Mode::PROJECTION): ASSERT(projection_stack.size() > 1); projection_stack.pop(); break;
        case (Matrix_Mode::MODELVIEW): ASSERT(modelview_stack.size() > 1); modelview_stack.pop(); break;
    }
}

FILDEF void translate (float x, float y)
{
    modelview_stack.top() = glm::translate(modelview_stack.top(), vec3(x,y,0));
}
FILDEF void rotate (float x, float y, float angle)
{
    modelview_stack.top() = glm::rotate(modelview_stack.top(), angle, vec3(x,y,0));
}
FILDEF void scale (float x, float y)
{
    modelview_stack.top() = glm::scale(modelview_stack.top(), vec3(x,y,1));
}
